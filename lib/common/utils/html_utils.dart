...
class HtmlUtils {
  static generateCode2HTml(String mdData, {String backgroundColor = UIcons.miWhiteString, String lang = 'java', userBR = true}) {
  ...
  static generateHtml(String mdData, {String backgroundColor = UIcons.webDraculaBackgroundColorString, userBR = true}) {
    if (mdData == null) {
      return "";
    }
    ...
    return generateCodeHtml(mdDataCode, false, backgroundColor: backgroundColor, actionColor: UIcons.actionBlueString, userBR: userBR);
  }

  /**
   * style for mdHTml
   */
  static generateCodeHtml(mdHTML, wrap, {backgroundColor = UIcons.white, String actionColor = UIcons.actionBlueString, userBR = true}) {
    return "<html>\n" +
        "<head>\n" +
        "<meta charset=\"utf-8\" />\n" +
        "<title></title>\n" +
        ...
        "tr:nth-child(even) {" +
        "background:" +
        UIcons.primaryLightValueString +
        ";" +
        "color:" +
        UIcons.miWhiteString +
        ";" +
  ...
  static resolveHtmlFile(var res, String defaultLang) {
    if (res != null) {
      ...
      int startLang = res.indexOf(startTag);
      int endLang = res.indexOf("\" data-path=\"");
      String lang;
      if (startLang >= 0 && endLang >= 0) {
        String tmpLang = res.substring(startLang + startTag.length, endLang);
        if (tmpLang != null) {
      ...
      if ('markdown' == lang) {
        return generateHtml(res, backgroundColor: UIcons.miWhiteString);
      } else {
        return generateCode2HTml(res, backgroundColor: UIcons.webDraculaBackgroundColorString, lang: lang);
      }
    } else {
